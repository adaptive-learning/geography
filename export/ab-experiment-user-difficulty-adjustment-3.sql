...
CREATE TABLE tmp_answer AS (
    SELECT
        ...
        ROUND((proso_common_config.content::json->'proso_models'->'item_selector'->'parameters'->>'target_probability')::FLOAT * 100) / 100 AS target_success
    ...
    INNER JOIN proso_common_config
        ON proso_models_answer.config_id = proso_common_config.id
    WHERE experiment_setup_id IN (41, 42, 43)
...
CREATE TABLE tmp_custom_config AS (
    SELECT
        custom.id,
        custom.condition_key,
        custom.condition_value,
        custom.user_id,
        config.app_name,
        config.key,
        config.content
    FROM proso_common_customconfig AS custom
    INNER JOIN proso_common_config AS config
        ON custom.config_id = config.id
);
\copy tmp_custom_config TO '/tmp/custom_config.csv' DELIMITER ',' CSV HEADER;
DROP TABLE tmp_custom_config;
