...
CREATE TABLE tmp_context AS (
    SELECT
        proso_flashcards_flashcard.id as flashcard_id,
...
CREATE TABLE tmp_options AS (
    SELECT
        proso_models_answer.id,
        proso_models_answer.item_asked_id,
        proso_models_answer.item_answered_id,
        proso_flashcards_flashcard.item_id as item_option_id,
        proso_configab_answerexperimentsetup.experiment_setup_id
    FROM proso_models_answer
    INNER JOIN proso_configab_answerexperimentsetup
        ON proso_models_answer.id = answer_id
    INNER JOIN proso_flashcards_flashcardanswer_options
        ON proso_flashcards_flashcardanswer_options.flashcardanswer_id = proso_models_answer.id
    INNER JOIN proso_flashcards_flashcard
        ON proso_flashcards_flashcard.id = proso_flashcards_flashcardanswer_options.flashcard_id
    WHERE experiment_setup_id IN (6, 7, 8, 9)
    ORDER BY proso_models_answer.id
);
\copy tmp_options TO '/tmp/options.csv' DELIMITER ',' CSV HEADER;
