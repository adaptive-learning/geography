...
def user_list_view(request):
    ...
    response = [s.to_serializable()
                for s in students if not is_lazy_user(s.user)]
...
def user_view(request):
    ...
    username = student.user.username if student is not None and not is_lazy_user(
        request.user) else ''
    points = student.points if student is not None else 0
    response = {
        'username': username,
        'points': points,
    }
